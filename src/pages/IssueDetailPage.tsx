    ...
    <div style={{ maxWidth: '1200px', margin: '32px auto', padding: '0 24px' }}>
      ...
      <div className="grid-timeline-layout" style={{ gap: '32px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {/* Admin / Warden Console Action Buttons (Universal testing mode) */}
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 600 }}>Warden Resolution Controls</h3>
            <p className="text-xs" style={{ color: 'var(--text-3)', margin: 0 }}>
              Testing features: Change issue progression states directly.
            </p>

            <div style={{ display: 'flex', gap: '8px' }}>
              <button 
                onClick={handleMarkInProgress}
                disabled={statusUpdating || issue.status !== 'reported' && issue.status !== 'verified'}
              ...
              <button 
                ...
                disabled={statusUpdating || issue.status === 'resolved'}
          ...
          {/* Resolution Proof Upload & AI Verification Card */}
          {issue.status !== 'resolved' && (
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px', border: '1px solid #10B981', background: 'rgba(16, 185, 129, 0.02)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <CheckCircle size={16} style={{ color: '#10B981' }} />
                <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#10B981' }}>Upload Proof of Resolution</h3>
              </div>
              <p className="text-xs" style={{ color: 'var(--text-2)', margin: 0 }}>
                Upload a site photo showing the completed repairs. Our **AI Image Comparison Agent** will analyze the before and after states to verify the fix.
              </p>

              {resolvedImagePreview ? (
                <div style={{ position: 'relative', width: '100%', aspectRatio: '16/10', borderRadius: '6px', overflow: 'hidden', border: '1px solid var(--border)' }}>
                  <img src={resolvedImagePreview} alt="Resolution proof preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <button 
                    onClick={() => { setResolvedImagePreview(null); setResolvedImage(null); }}
                    style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(0,0,0,0.8)', color: '#FFF', border: 'none', borderRadius: '50%', width: '24px', height: '24px', cursor: 'pointer', fontSize: '11px' }}
                  >
                    ✕
                  </button>
                </div>
              ) : (
                <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '20px', border: '1px dashed #10B981', borderRadius: '6px', cursor: 'pointer', transition: 'background 0.2s' }} className="hover-card">
                  <Camera size={24} style={{ color: '#10B981' }} />
                  <span style={{ fontSize: '12px', color: '#10B981', fontWeight: 500 }}>Select Proof Photo</span>
              ...
              {resolvedImagePreview && (
                <button
                  ...
                  className="btn btn-primary"
                  style={{ width: '100%', background: '#10B981', borderColor: '#10B981', justifyContent: 'center' }}
                >
                  {verifyingResolution ? (
                    <>
                      <Loader size={14} className="shimmer" style={{ animation: 'spin 1s linear infinite' }} />
                      AI Comparing Images...
                    </>
                  ) : (
                    "Verify with Resolution Agent"
                  )}
                </button>
              )}
            </div>
